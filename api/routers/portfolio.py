...
def _blended_inner(conn, strategy: str, starting: float, years: float):
    """Delegates to the one shared definition — see framework/analysis/blended.py.

    Kept as a named function because `_blended_and_benchmark` above opens its
    own connection and this is the seam the tests and the sweep both use.
    """
    from framework.analysis.blended import blended_and_benchmark
    return blended_and_benchmark(conn, strategy, starting, years)
