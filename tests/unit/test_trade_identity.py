...
def test_insert_errors_are_no_longer_all_called_duplicates():
    """The handler read `except Exception: pass  # duplicate` and counted
    schema errors, type errors and real constraint violations alike as
    filings we already had."""
    i = PARSER.index("INSERT OR IGNORE INTO trades")
    tail = PARSER[i:i + 4000]
    assert "pass  # duplicate" not in tail, "the blind handler is back"
    assert "logger.error" in tail, (
        "a non-duplicate insert failure is silent again"
    )
