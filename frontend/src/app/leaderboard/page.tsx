...
export default async function LeaderboardPage({ searchParams }: Props) {
  ...
  function buildUrl(overrides: Record<string, string> = {}) {
    const merged = { sort, order, min_trades: minTrades, min_tier: minTier, min_rating: minRating, active, ...overrides };
    ...
    <div>
      ...
      <div className="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          ...
          {/* Rating. These were "Tier 2+" / "Tier 3+", filtering on
              itr.score_tier — an all-time figure recomputed on every refresh,
              which is not a thing you can filter a point-in-time leaderboard
              by. Same two steps, named after the rating shown in the table. */}
          {["A", "A+"].map((r) => (
            <Link
              key={r}
              href={buildUrl({ min_rating: minRating === r ? "" : r, min_tier: "" })}
              className={`rounded-md px-3 py-1.5 text-xs font-medium border transition-colors ${
                minRating === r
                  ? "border-[#3B82F6]/50 bg-[#3B82F6]/10 text-[#3B82F6]"
                  : "border-[#2A2A3A] text-[#8888A0] hover:text-[#E8E8ED]"
